...
export default function LatestWorkVariant() {
  ...
  return (
    <section className="flex flex-col w-full bg-white text-ink relative overflow-hidden font-[family-name:var(--font-dm-sans)] pb-8">
  ...
  {filteredProjects.map((project) => (
    <div
      key={project.id}
      className="group bg-white rounded-3xl cursor-pointer border border-black/[0.06] [box-shadow:rgba(0,0,0,0.35)_0px_4px_8px,rgba(0,0,0,0.35)_0px_30px_50px_-10px,rgba(0,0,0,0.25)_0px_60px_100px_-20px,rgba(0,0,0,0.18)_0px_-3px_0px_inset] hover:[box-shadow:rgba(0,0,0,0.4)_0px_6px_12px,rgba(0,0,0,0.4)_0px_45px_70px_-12px,rgba(0,0,0,0.3)_0px_80px_130px_-24px,rgba(0,0,0,0.22)_0px_-3px_0px_inset] transition-shadow duration-300 transform hover:-translate-y-1"
    >
      ...
      <div className="px-3 flex flex-wrap gap-2">
        {project.tags.map((tag) => (
          <span
            key={tag}
            className="px-3 py-1 text-xs font-[family-name:var(--font-dm-sans)] tracking-widest bg-black/10 text-black rounded-full uppercase"
          >
            {tag}
          </span>
        ))}
      ...
      <div className="p-3 flex justify-between items-center">
        ...
        <span className="flex items-center gap-1 text-sm font-medium" style={{ color: 'var(--color-accent)' }}>
          View Project
